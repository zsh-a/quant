...
// curl http://localhost:8000/backtest/sh.000001/20201010/20211010
// 自定义覆盖物模板
const customAnnotation: OverlayTemplate = {
  name: 'customAnnotation',
  totalStep: 1,
  createPointFigures: ({ overlay, coordinates }) => {
    const text = overlay.extendData === 'buy' ? '^' : 'v';
    const color = overlay.extendData === 'buy' ? 'green' : 'red';
...
// 注册自定义覆盖物模板
registerOverlay(customAnnotation);
const BuySellPoint: OverlayTemplate = {
  name: 'BuySellPoint',
  totalStep: 2,
  styles: {
    line: { style: 'dashed' },
  ...
  createPointFigures: ({ overlay, coordinates }) => {
    ...
    const lineEndY = text === 'buy' ? startY + 50 : startY - 50
    const arrowEndY = text === 'buy' ? lineEndY + 5 : lineEndY - 5
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log(data)
        let new_kline = data['kline'].map((item: any) => {
        ...
        console.log(new_kline)
        chart.applyNewData(new_kline);
        // 处理数据
      } catch (error) {
        // 错误处理
      }
    ...
    // chart.createIndicator("MA", { params: [5, 10, 20] }); // 5日、10日、20日均线
