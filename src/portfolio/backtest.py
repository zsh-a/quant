...
class PortfolioBacktester:
    ...
    def __init__(self, portfolio_manager: PortfolioManager, initial_capital: float = 1000000.0):
    ...
    def run(self, start_date: str, end_date: str, symbols: List[str]) -> PortfolioBacktestResult:
        ...
        for strategy_name, weight in weights.items():
            ...
            self.brokers[strategy_name] = BacktestBroker(initial_capital=capital, commission_rate=0.0003)
            logger.debug(f"Strategy {strategy_name}: ${capital:,.0f} ({weight:.1%})")

        # Create data stream
        data_stream = DBDataStream(symbols=symbols, start_date=start_date, end_date=end_date)
    ...
    def _execute_signal(self, strategy_name: str, signal: Dict, bar: Dict, broker: BacktestBroker):
    ...
    def _rebalance(self, current_date: str):
        ...
        # Record weights
        weights = self.portfolio_manager.get_weights()
        self.weights_history.append({"date": current_date, "weights": weights.copy()})
    ...
    def _record_equity(self, current_date: str):
        ...
        self.equity_history.append(
            {"date": current_date, "total_equity": total_equity, "strategy_equities": strategy_equities}
        )
