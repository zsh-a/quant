...
_TICK = 1e-6  # generic — broker's _get_tick_size applied at order time
...
class _PBState:
    ...
    pb_start_idx: int = -1
    leg1_extreme: float = 0.0  # leg1 low for H2, leg1 high for L2
    recovery_extreme: float = 0.0  # recovery high for H2, recovery low for L2
...
class _TwoLeggedPullbackBase:
    ...
    def on_bar(self, ctx: DetectorContext) -> Optional[PatternSignal]:
        if not self._trend_ok(ctx):
            # If we lose the trend backdrop, reset.
            self._s = _PBState()
            return None
        ...
        if s.state == "H2_FORMED":
            # External trigger (stop order) handled by Strategy; we time out
            # if not triggered within 3 bars.
    ...
    def mark_triggered(self, bar_idx: int) -> None:
        """Called by Strategy when entry fills to move FSM out of H2_FORMED."""
    ...
    def _is_against(self, feat) -> bool:  # pragma: no cover
        """True if this bar moves against the trend (leg direction)."""
        raise NotImplementedError
    ...
    def _extreme_against(self, feat, current: float) -> float:  # pragma: no cover
        """For H2: min(low, current leg1_low); for L2: max(high, current leg1_high)."""
        raise NotImplementedError
    ...
    def _recovery_broken(self, feat) -> bool:  # pragma: no cover
        """Recovery gets exceeded → flag not H2 — reset to IDLE."""
        raise NotImplementedError
    ...
    def _handle_leg1(self, ctx: DetectorContext) -> None:
        feat = ctx.feat
        s = self._s
        # ATR-based invalidation: pullback too deep → not a H2 setup
        atr = ctx.feat.atr14
        if self.side == "long":
            # If close drops far below pb_start by >2 ATR, give up
            start_close = (
    ...
    def _handle_recovery(self, ctx: DetectorContext) -> None:
        ...
        if self._recovery_broken(feat):
            # Recovery taken out → no H2 yet; reset.
            self._s = _PBState()
            return
    ...
    def _handle_leg2(self, ctx: DetectorContext) -> Optional[PatternSignal]:
        ...
        if self._leg2_breaks_leg1(feat):
            # Signal bar must be a *with-trend* bar or a reversal bar to form H2.
            ...
            # else wait one more bar for a reversal
        return None
...
class H2Detector(_TwoLeggedPullbackBase):
    ...
    def _recovery_broken(self, feat) -> bool:
        # If we break above prior swing high with conviction, the pullback is over.
        return False  # rely on leg2 path; over-eager reset is worse than late invalidation
