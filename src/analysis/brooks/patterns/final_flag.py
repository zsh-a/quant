"""Final Flag detector — tight range after a strong trend leg, then break the
*opposite* direction (trend exhaustion).

Rules (simplified):
  * Preceding strong leg: trend has ``leg_length >= leg_min`` bars
  * Consolidation: ``flag_min_bars`` bars of tight range (range < flag_atr_mult × ATR)
...
class FinalFlagDetector:
    ...
    def on_bar(self, ctx: DetectorContext) -> Optional[PatternSignal]:
        ...
        # Scan for a flag window ending at previous bar.
        for flag_len in range(self.flag_min_bars, min(self.flag_max_bars, len(recent) - self.leg_min - 1) + 1):
            flag_slice: List = recent[-flag_len - 1 : -1]  # exclude current bar
            ...
            if pre_range < 2 * flag_range:
                continue  # preceding leg not strong enough relative to flag

            # Decide trend direction from pre-flag: rising or falling close
            pre_trend_up = pre_slice[-1].close > pre_slice[0].close
            # Final flag *fails* the trend → breakout opposite trend.
            if pre_trend_up and feat.close < flag_low:
                ...
                return PatternSignal(
                    ...
                    metadata={"flag_high": flag_high, "flag_low": flag_low, "flag_len": flag_len},
                )
