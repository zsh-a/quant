...
{{"reason": "one-sentence justification", "score": <integer 0-10>}}"""
    ...
    # ------------------------------------------------------------------
    # Legacy interface (backward compat with old MCTSRefinementStrategy)
    # ------------------------------------------------------------------
    ...
    def get_refinement_suggestion(
        ...
        """Legacy: generate a refinement suggestion."""
        suggestion, _ = self.generate_refined_alpha(
            parent_formula=formula,
            target_dimension=dimension.split("(")[0].strip().lower(),
            eval_scores={},
    ...
    def refine_alpha(
        ...
        """Legacy: generate a refined formula based on suggestion."""
        _, new_formula = self.generate_refined_alpha(
            parent_formula=formula,
