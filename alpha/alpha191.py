...
def Rank(sr):
    # 列-升序排序并转化成百分比
    return sr.rank(axis=0, method="min", pct=True)
...
class Alphas191:
    ...
    def alpha055(self):  # 公式有问题
        ...
        part1[cond3] = (
            Abs(self.high - Delay(self.low, 1))
            + Abs(Delay(self.close, 1) - Delay(self.open, 1)) / 4
        )
        part2 = Max(
            Abs(self.high - Delay(self.close, 1)), Abs(self.low - Delay(self.close, 1))
        )
    ...
    def alpha056(self):
        ...
        B = Rank(
            (
                Rank(
                    Corr(
                        Sum(((self.high + self.low) / 2), 19),
                        Sum(Mean(self.volume, 40), 19),
                        13,
                    )
                )
                ** 5
    ...
    def alpha075(self):
        ...
        return Count(
            ...
            50,
        ) / Count((self.benchmark_close < self.benchmark_open), 50)
    ...
    def alpha101(self):
        ...
        rank2 = Rank(
            Corr(Rank(((self.high * 0.1) + (self.vwap * 0.9))), Rank(self.volume), 11)
        )
        cond = rank1 < rank2
        part = self.close.copy(deep=True)
        part.loc[:, :] = None
    ...
    def alpha123(self):
        ...
        A = Rank(
            Corr(
                Sum(((self.high + self.low) / 2), 20), Sum(Mean(self.volume, 60), 20), 9
            )
        )
    ...
    def alpha148(self):
        ...
        cond = Rank(Corr((self.open), Sum(Mean(self.volume, 60), 9), 6)) < Rank(
            (self.open - Tsmin(self.open, 14))
        )
        part = self.close.copy(deep=True)
        part.loc[:, :] = None
        part[cond] = -1
    ...
    def alpha154(self):
        ...
        cond = (self.vwap - Tsmin(self.vwap, 16)) < (
            Corr(self.vwap, Mean(self.volume, 180), 18)
        )
        part = self.close.copy(deep=True)
        part.loc[:, :] = None
        part[cond] = 1
    ...
    def alpha172(self):
        ...
        TR = Max(
            Max(self.high - self.low, Abs(self.high - Delay(self.close, 1))),
            Abs(self.low - Delay(self.close, 1)),
        )
        ...
        return Mean(
            ...
            / (Sum(part1, 14) * 100 / Sum(TR, 14) + Sum(part2, 14) * 100 / Sum(TR, 14))
            * 100,
            6,
        )
    ...
    def alpha186(self):
        ...
        return (
            Mean(
                Abs(
                    Sum(part1, 14) * 100 / Sum(TR, 14)
                    - Sum(part2, 14) * 100 / Sum(TR, 14)
                )
                / (
                    Sum(part1, 14) * 100 / Sum(TR, 14)
                    + Sum(part2, 14) * 100 / Sum(TR, 14)
...
if __name__ == "__main__":
    ...
    query = """
    SELECT code
    FROM stock_data.stock_daily AS sd
    WHERE 
        sd.code IN (
            SELECT code
            FROM stock_data.stock_daily_meta
            WHERE code LIKE '%.00%' OR code LIKE '%.60%' OR code LIKE '%.30%'
        )
        AND sd.isST != 1 -- 筛选非 ST 的股票
        AND date = (SELECT MAX(last_update_date) FROM stock_data.stock_daily_meta)
        AND close < 50
    ORDER BY code
    """

    def alpha001(volume, close, open):  # 平均1751个数据
        ##### (-1 * CORR(RANK(DELTA(LOG(VOLUME), 1)), RANK(((CLOSE - OPEN) / OPEN)), 6))####
    ...
    for index, row in df.iterrows():
        # print(row['code'])
        # print(alpha001(row['volume'],row['close'],row['open']))
    ...
    # print(duplicates)  # 显示重复的索引
    assert df.index.is_unique

    df["month"] = df.index.get_level_values("date").to_period("M")

    df["future_return"] = df.groupby(level=0)["close"].transform(
        lambda x: np.log(x.shift(-5) / x)
    )

    df["alpha001"] = (
        df.groupby("code")
        .apply(lambda group: alpha001(group["volume"], group["close"], group["open"]))
        .reset_index(level=0, drop=True)
    )

    # 1. 对因子进行排序，按升序（或降序）分组
    df["factor_rank"] = df.groupby("date")["alpha001"].rank(ascending=False)
    df["factor_group"] = pd.qcut(
        df["factor_rank"], q=5, labels=["1", "2", "3", "4", "5"]
    )
    print(df)

    trading_days = df.index.get_level_values('date').unique()

    rebalance_days = trading_days.unique()[::5]  # 调仓日期

    # 回测参数
    initial_capital = 100000  # 初始资金
    position_size = 10  # 每次持仓数量
    transaction_cost = 0.001  # 单次交易成本


    position_size = 5  # 每次持仓数量
    positions = {}  # 持仓字典：key是股票，value是持仓数量
    portfolio_value = initial_capital
    # 存储每日收益记录
    performance = []

    # 回测逻辑
    for i, day in enumerate(sorted(trading_days.unique())):
        # 获取T日数据
        day_data = df.xs(day, level='date')
        # 如果是调仓日
        if day in rebalance_days and i < len(df.index.get_level_values('date').unique()) - 1:
            # 获取T+1日数据
            next_day = sorted(df.index.get_level_values('date').unique())[i + 1]
            next_day_data = df.xs(next_day, level='date')

            # 生成T日的交易信号（选取因子排名靠前的股票）
            selected_stocks = day_data.nsmallest(position_size, 'factor_rank')
            print(day,selected_stocks)

            # 卖出持仓
            for stock in list(positions.keys()):
                stock_price = next_day_data.loc[stock]['close'] if stock in next_day_data.index else None
                if stock_price is not None:  # 确保股票在T+1日有价格
                    portfolio_value += stock_price * positions[stock]  # 卖出金额
                    portfolio_value -= transaction_cost * stock_price * positions[stock]  # 减去交易成本
                    del positions[stock]  # 清空持仓

            # 买入新的股票
            for stock, row in selected_stocks.iterrows():
                if stock in next_day_data.index:
                    stock_price = next_day_data.loc[stock]['close']
                    shares = (portfolio_value / position_size) // stock_price  # 每个股票的持仓股数
                    positions[stock] = shares
                    portfolio_value -= shares * stock_price  # 扣除买入金额
                    portfolio_value -= transaction_cost * shares * stock_price  # 扣除交易成本

        # 计算每日收益
        daily_value = portfolio_value + sum(
            day_data.loc[stock]['close'] * shares
            for stock, shares in positions.items() if stock in day_data.index
        )

        performance.append({'date': day, 'portfolio_value': daily_value})
    ...
    plt.plot(performance['date'], performance['portfolio_value'], label='Portfolio Value')
    plt.xlabel('Date')
    ...
    grouped_returns = df.groupby("factor_group")["future_return"].mean()
    # 查看不同因子组的未来收益
    print(grouped_returns)

    df = df.replace([np.inf, -np.inf], np.nan)  # 将无穷值替换为 NaN
    df.dropna(inplace=True)

    print(df.isna().sum())  # 检查缺失值
    print(df.isin([np.inf, -np.inf]).sum())  # 检查正负无穷值

    def calculate_ic(group):
        # 对于每只股票，按月份分组
        return group.groupby("month").apply(
            lambda month_group: month_group["alpha001"].corr(
                month_group["future_return"]
            ),
            include_groups=False,
        )

    ic = df.groupby("code").apply(calculate_ic)
    # 强制结果为MultiIndex
    if not isinstance(ic.index, pd.MultiIndex):
        # 如果不是MultiIndex，手动转换
        ic = ic.stack().reset_index()
        ic.columns = ["code", "month", ""]
        ic.set_index(["code", "month"], inplace=True)
    # 确保索引名称一致
    ic.index.names = ["code", "month"]

    # print(ic.index)
    # ic.reset_index(['code', 'month'], inplace=True)
    monthly_mean_ic = ic.groupby(level="month").mean()  # 按列（每个月）计算均值
    print(monthly_mean_ic)
    ...
    monthly_mean_ic.plot(kind="line", marker="o", color="b")
    ...
    plt.savefig("monthly_ic_plot.png", dpi=300, bbox_inches="tight")  # 保存图像

    # def calculate_ic(group):
    #     return group['close'].corr(group['future_return'])
    # ic_values = df.groupby('code').apply(calculate_ic)
    # print(ic_values)
    # res_df.to_csv('alpha001.csv',index=False)

    # df = pd.read_csv('alpha001.csv')

    # # sort by alpha001
    # df = df.sort_values(by='alpha001', ascending=True)
    ...
    # alpha = Alphas191(ds.data)
    # df = ds.data['close']
    # a001 = alpha.alpha001()
    # # print(alpha.alpha001())
    # returns = df.rolling(2).apply(lambda x: np.log(x.iloc[-1] / x.iloc[0]))

    # future_return = returns.rolling(window=5, min_periods=1).apply(np.sum, raw=True).shift(-5)
    # future_return = future_return.apply(np.exp) - 1
    # # print(alpha.returns)
    # print(a001.corr(future_return))

    # print(a001.to_csv('alpha191.csv'))
    # Rank()
    # print(Delta(log(ds.data['close']), 1))
