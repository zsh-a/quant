...
class OrderManager:
    def __init__(self, account: Account, order_policy):
        ...
        self.buy_waiting_orders = []
        self.sell_waiting_orders = []
    ...
    def create_order(self, symbol, order_type, quantity, price=None, exec_time="open"):
        ...
        logger.info(
            f"create order | datetime : {self.get_current_timestamp()} | symbol : {symbol} | name : {name} | order_id : {order.order_id}  | order_type : {order_type} | quantity : {quantity} | exec_time : {exec_time}"
        )
        self.order_id_counter += 1
        return order
