...
up() {
  ...
  start_background "api" "$ROOT_DIR" \
    "export PYTHONPATH='$PYTHONPATH' SESSION_DB_PATH='$SESSION_DB_PATH' CELERY_BROKER_URL='$CELERY_BROKER_URL' CELERY_RESULT_BACKEND='$CELERY_RESULT_BACKEND' PYTHONUNBUFFERED=1; '$ROOT_DIR/.venv/bin/python' -m uvicorn src.api.server:app --reload --host 0.0.0.0 --port '$API_PORT'"

  start_background "worker" "$ROOT_DIR" \
    "export PYTHONPATH='$PYTHONPATH' SESSION_DB_PATH='$SESSION_DB_PATH' CELERY_BROKER_URL='$CELERY_BROKER_URL' CELERY_RESULT_BACKEND='$CELERY_RESULT_BACKEND' PYTHONUNBUFFERED=1; '$ROOT_DIR/.venv/bin/celery' -A src.tasks.celery_app worker --loglevel=info --concurrency='$CELERY_WORKER_CONCURRENCY' --queues='$CELERY_QUEUES' --max-tasks-per-child=100 --time-limit=3600 --soft-time-limit=3000"
