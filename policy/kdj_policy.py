...
class BaseOrderPolicy(OrderPolicy):
    ...
    def buy_cond(self, code):
        ...
        # print(self.cur_obs[idx])
        return self.cur_obs[idx][key] > self.last_obs[-1][idx]["high"] and self.cur_obs[idx]["D"] > self.last_obs[-1][idx]["D"] and self.last_obs[-1][idx]["D"] < 30
    ...
    def sell_cond(self, code):
        if len(self.last_obs) < 2 or self.last_obs[-2] is None:
            return False
        idx = global_var.SYMBOLS.index(code)
