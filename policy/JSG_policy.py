...
class OrderPolicy(OrderPolicy):
    def __init__(self, account, max_stocks=NUM_STOCKS, **args) -> None:
        self.last_obs = []
        self.cur_obs = None
    ...
    def check_up_down_limit(self, code, key):
        ...
        if not df.empty and "code" in (df.index.names if hasattr(df.index, 'names') else []):
