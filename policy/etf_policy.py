...
class Agent:
    def __init__(self, market_env: MultiMarketEnv, **args) -> None:
        ...
        self.db_client = args["db_client"] if "db_client" in args else None
    ...
    def get_etf_rank(self):
        ...
        for etf in self.etf_pool:
            df = dfs.loc[dfs.index.get_level_values('code') == etf]
            if len(df) == 0:
                continue
            # logger.info(f"{df}")
            df.reset_index(level="date", drop=True, inplace=True)
...
if __name__ == "__main__":
    agent = Agent(None)
    print(agent.action_decider(None))
